...
import { TIMESHEET_STATUS } from "../../Constant";

class TimesheetStatusMain extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  getTimesheetCount = async () => {
    ...
    let result = await httpClient("timesheet/count", "GET");
    if (result.success) {
      this.setState({
      ...
      this.props.selectedStatus
        ...
        : this.props.onStatusSelected("WithEmployee");
    } else {
...
export default TimesheetStatusMain;
